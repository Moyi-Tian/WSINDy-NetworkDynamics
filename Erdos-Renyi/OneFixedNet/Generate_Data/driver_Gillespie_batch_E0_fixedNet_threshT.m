...
tempdir=pwd;
save_network_path=[tempdir,'/networks'];
network_fname = sprintf('Erdos-Renyi Network N=%u, k=%u - %u.mat',N,k,network_id);
save_network_flag = 1;
if exist(fullfile(save_network_path, network_fname),'file') == 2
    fprintf('Network already exists. Loading... \n')
    load(fullfile(save_network_path, network_fname));
    save_network_flag = 0;
else
...
X5_reduced_cell = cell(num_ICs, 1);
X4_reduced_cell = cell(num_ICs, 1);
X3_reduced_cell = cell(num_ICs, 1);
...
for IC_id = 1:num_ICs
    ...
    X5_reduced_cell{IC_id} = [U_mean', E_mean', D_mean', P_mean', R_mean'];
    X4_reduced_cell{IC_id} = [U_mean', E_mean', P_mean', R_mean'];
    X3_reduced_cell{IC_id} = [U_mean', E_mean', P_mean'];
...
if save_data == 1
    ...
    fname = sprintf('Dynamics from Gillepsie batched E0 on Fixed Erdos-Renyi N=%u, k=%u, init_E=%.5g-%.5g, num_IC=%u, beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, tend=%.3g, num_t=%u, iter=%u_ThreshT - NetID-%u.mat',N,k,Initial_E_percents(1),Initial_E_percents(end),num_ICs,beta,theta,eta,gamma_i,gamma_p,t_max,t_end,NumGrids,NumIteration,network_id);
    save(fullfile(save_path, fname), 'G', 'parameters', 'times_all', ...
                                     'U_all', 'E_all', 'D_all',...
                                     'UP_all', 'P_all', 'R_all',...
                                     't_grid', 'X6_cell', 'X5_reduced_cell',...
                                     'X4_reduced_cell', 'X3_reduced_cell',...
                                     'X6_mins', 'X6_maxes');
    
     % also separately save the network if we created it...
    if save_network_flag == 1
        save(fullfile(save_network_path, network_fname), 'G');
    end
