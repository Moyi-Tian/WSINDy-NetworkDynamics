...
%% Output Error Heatmap for E and P only
num_trajs_upto_data = size(trajectory_errors_cell, 1);
numNoise = size(trajectory_errors_cell, 2);
Repeat = size(trajectory_errors_cell, 3);
...
for eq = eq_to_plot
    ...
    for i = 1:plot_trajNum_to
        for j = 1:numNoise
            vals_ij = arrayfun(@(r) trajectory_errors_cell{i,j,r}(eq), 1:Repeat);
    ...
    if eq == 2
        cb.Label.String = 'Median Output Error';
    else
        cb.Label.String = '';
    end
    ...
    filename = sprintf('%s_OutputErr_heatmap_median_%uNoises_%uRepeat_no_cutoff.pdf', ...
