%% Appendix boxplots: equation error cross-section at one noise level
...
%% Extract equation error values
% EqErrVals has dimensions:
% equation x number of trajectories x repeat
EqErrVals = NaN(nEq, plot_trajNum_to, rept);

for eq = eq_to_plot
    for nt = 1:plot_trajNum_to
        for r = 1:rept
            curVal = EquationErr_cell{nt, noise_idx, r};
            if ~isempty(curVal) && numel(curVal) >= eq
                EqErrVals(eq, nt, r) = curVal(eq);
            end
        end
    end
end
...
%% Generate one boxplot per equation, E and P only
SummaryRows = {};

for eq = eq_to_plot

    letter = eq_letters{eq};
    vals = squeeze(EqErrVals(eq, :, :));
    ...
    %% Save figure
    filename = sprintf('%s_EquationErr_CrossSection_Boxplot_noise_%s.pdf', ...
        letter, alpha_str);
...
SummaryTable = cell2table(SummaryRows, ...
    'VariableNames', {'Equation', 'NoiseLevel', 'NumTrajectories', ...
    'Median', 'Q25', 'Q75', 'IQR'});

disp(SummaryTable)

writetable(SummaryTable, fullfile(plot_path, ...
    sprintf('EquationErr_CrossSection_SummaryTable_EP_only_noise_%s.csv', alpha_str)));