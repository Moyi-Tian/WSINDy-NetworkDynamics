...
fname = sprintf('Fully-Mixed Network Dynamics batch %u E0=%.5g-%.5g, beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, t_grid_num=%u.mat',length(E0),E0(1),E0(end),beta,theta,eta,gamma_i,gamma_p,time_range(end),t_num);
load(fname);

start_val = 0.01;
end_val = 0.3;
J = find(E0 >= start_val & E0 <= end_val);
ICs = E0(J);
num_ICs = numel(J);
num_trajs_upto = 10;
    ...
    for noise_i=1:numNoise
        ...
        for r=1:rept
                ...
                for q=toggle_compare
                    ...
                    for l=1:nstates
                        ...
                        if save_figs == 1
                            ...
                            % Save figure
                            fig_folder = sprintf('figures_3StateInput_numTrajUpTo_%u_IC_E0_=%.5g-%.5g_NumNoise=%u_rept=%u_noise_%.5g-%.5g_MSTLS', num_trajs_upto,start_val,end_val,numNoise,rept,noise_ratios(1),noise_ratios(end));
            ...
            if save_log == 1
                ...
                filename = sprintf('Results_3StateInput_numTrajUpTo_%u_IC_E0_=%.5g-%.5g_NumNoise=%u_rept=%u_noise_%.5g-%.5g_MSTLS.txt', num_trajs_upto,start_val,end_val,numNoise,rept,noise_ratios(1),noise_ratios(end));
...
if save_result == 1
    ...
    if exist('true_nz_weights', 'var')
        fname = sprintf('WSINDy Results for Fully-Mixed Network Dynamics E0 Batch Data numTrajUpTo=%u, E0_=%.5g-%.5g, beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, t_grid_num=%u, NumNoise=%u, repeat=%u_noise_%.5g-%.5g_MSTLS_Wboth.mat',num_trajs_upto,start_val,end_val,beta,theta,eta,gamma_i,gamma_p,time_range(end),t_num,numNoise,rept,noise_ratios(1),noise_ratios(end));
        weights_true = w_true;
        ...
        fname = sprintf('WSINDy Results for Fully-Mixed Network Dynamics E0 Batch Data numTrajUpTo=%u, E0_=%.5g-%.5g, beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, t_grid_num=%u, NumNoise=%u, repeat=%u_noise_%.5g-%.5g_MSTLS_Wlr.mat',num_trajs_upto,start_val,end_val,beta,theta,eta,gamma_i,gamma_p,time_range(end),t_num,numNoise,rept,noise_ratios(1),noise_ratios(end));
        save(fullfile(save_path, fname),...
                    'Tps_cell', 'CoeffErr_cell', 'EquationErr_cell', ...
                     'radii_cell','G_CondNum_cell','trajectory_errors_cell',...
